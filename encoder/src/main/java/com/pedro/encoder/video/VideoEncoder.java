...
public class VideoEncoder extends BaseEncoder implements GetCameraData {
  ...
  public boolean prepareVideoEncoder(int width, int height, int fps, int bitRate, int rotation,
    ...
    try {
      ...
      if (formatVideoEncoder == FormatVideoEncoder.SURFACE
        ...
        Log.d("codepixels", codec.getCodecInfo().toString());
        Log.i("codepixels", Integer.toString(this.formatVideoEncoder.getFormatCodec()));
        Log.d("codepixels", Integer.toString(MediaCodecList.ALL_CODECS));
      }
      Log.i(TAG, "prepared");
      return true;
    } catch (Exception e) {
  ...
  public void start(boolean resetTs) {
    ...
    if (formatVideoEncoder != FormatVideoEncoder.SURFACE) {
      Log.d("codepixels", "notsurface");
      Log.d("codepixels", formatVideoEncoder.toString());
      YUVUtil.preAllocateBuffers(width * height * 3 / 2);
    }
    Log.d("codepixels", "yessurface");
    Log.i(TAG, "started");
  }
